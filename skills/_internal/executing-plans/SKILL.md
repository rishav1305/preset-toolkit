...
A plan should exist from `_internal/writing-plans` or the user's direct instructions. If no plan exists, suggest creating one first.
...
5. **If approved, push** -- invoke `_internal/sync-push` with the appropriate mode.
