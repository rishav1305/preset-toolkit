...
Invoke the `_internal/sync-pull` skill to pull, dedup, and verify.
...
Invoke the `_internal/validate` skill to run health checks.
...
Invoke the `_internal/screenshot` skill to capture current state.
...
Invoke the `_internal/visual-regression` skill to compare against baselines.
